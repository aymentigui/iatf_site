...
import { createhotels } from "@/actions/hotels/set"
import { getHotels } from "@/actions/hotels/get"
import { deleteHotel } from "@/actions/hotels/delete"
import { getColumns } from "@/actions/util/sheet-columns/hotels"
...
export function List() {
  ...
  useEffect(() => {
    if (sheetData && sheetData.length > 0) {
      createhotels(sheetData).then((res) => {
        if (res.status === 200) {
          if (res.data.hotels) {
          ...
          {sheetNotCreated.map((data: any, index: any) => (
            <div key={index} className="mt-4 p-4 bg-red-200 text-red-700 rounded">
              ...
              <ul className="list-disc pl-5">
                <li>
                  {
                    (data.message ? data.message + " : " : "") + " " + (data.hotel.nom_fr ?? "") 
                  }
                </li>
              </ul>
            </div>
              ...
              {loading ? (
                ...
                data.map((item) => (
                  <TableRow key={item.id}>
                    <TableCell className="font-medium">{item.nom_fr}</TableCell>
                    <TableCell>{item.adresse_fr || "-"}</TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end space-x-2">
                        <Button variant="ghost" size="sm" asChild>
                          <Link href={`/admin/hotels/${item.id}`}>
